...
class Keithley2470:
    def __init__(self):
        ...
        self._ilimit = 5e-3  # Current limit in A - now 1.5 mA
        ...
        #self._write(f"SOURCe:CLEar:AUTO ON")
        #self._write(f"SENSe:FUNCtion:CONCurrent OFF")
        ...
        self.bv_ramp_step = 50.   # 25 old value
        self.bv_ramp_wait = 0.5   # 0.5 old 
    ...
    def _read_buffer(self):
        ...
        status = True
        while status:
            print(self._query("TRIGger:STATe?"))
            status = self._query("TRIGger:STATe?").split(';')[0] != 'IDLE'
            sleep(3)
        sleep(5)
        start_index = self._query('TRACe:ACTual:STARt? "testData"')
        end_index = self._query('TRACe:ACTual:END? "testData"')
        print(start_index, end_index)
        readings = self._query('TRACe:DATA? ' + start_index + ', ' + end_index + ', "testData", READ, SOUR')
        print("Readings : ", readings)
        reading_list_current = readings.split(',')[::2]
        reading_list_voltage = readings.split(',')[1::2]
        reading_list = [{'current': reading_list_current[i], 'voltage': reading_list_voltage[i], 'resistance': float(reading_list_voltage[i])/float(reading_list_current[i])} for i in range(len(reading_list_current))]
    ...
    def set_output_enable(self, onoff):
        ...
        """
        if onoff:
            self._write(f"OUTPut:ENABle ON")
        else:
            self._write(f"OUTPut:ENABle OFF")
    ...
    def set_current_limit(self, ilimit):
        ...
        if self._CURRENT_LIMIT_LOW <= abs(ilimit) <= self._CURRENT_LIMIT_HIGH:
            self._ilimit = ilimit
            self._write(f"SOURce:VOLTage:ILIMit {ilimit}")
            self._inst.compliance_current = ilimit
        else:
    ...
    def set_voltage_limit(self, vlimit):
        ...
        if self._VOLTAGE_LIMIT_LOW <= abs(vlimit) <= self._VOLTAGE_LIMIT_HIGH:
            self._vlimit = vlimit
            self._inst.compliance_voltage = vlimit
        else:
    ...
    def get_limit_tripped(self):
        ...
        """
        vtrip = int(self._query(f"SOURce:VOLTage:PROTection:TRIPped?"))
        itrip = int(self._query(f"SOURce:CURRent:PROTection:TRIPped?"))
    ...
    def set_source_voltage(self, value):
        ...
        if self._VOLTAGE_LIMIT_LOW <= abs(value) <= self._vlimit:
            self._write(f"SOURce:FUNCtion VOLTage")
            ...
            if abs(difference) > self.bv_ramp_step:
                for i in range(1, int(abs(difference) // self.bv_ramp_step) + 1):
                    ...
                    self._write(f"SOURce:VOLTage {this_voltage}")
                    sleep(self.bv_ramp_wait)
            self.voltage_now = value
            self._write(f"SOURce:VOLTage {value}")
        else:
    ...
    def get_sense_current(self):
        ...
        #if not self.high_i_range:
        #    self._write(f"SENSe:CURRent:DC:RANG 100E-6")
        #else:
        #    self._write(f"SENSe:CURRent:DC:RANG 1E-3")

        self._write(f"SENSe:CURRent:DC:RANG:AUTO ON")
        start = time()
        while True:
        ...
        #measurement = self._query("MEASure:CURRent:DC?")    
        measurement = self._query("READ?", 0.)
        #print("Measured  " , self._parse_data(measurement)[0]['current'])
        return float(measurement)
    ...
    def voltage_sweep(self, Vmin, Vmax, steps, Ilimit=1.5e-3, delay_s=1.):
        ...
        if Vmin >= self._VOLTAGE_LIMIT_LOW and Vmax <= self._vlimit:
            ...
            #self.set_source_voltage(0.)
            #self._write('TRAC:MAKE "testData", 100')
            self._write('TRACe:CLEar "testData"')

            Vstep = (Vmax - Vmin) / steps 
            self.set_sense_mode("current")
            #self.set_current_limit(Ilimit)
            self._write(f"SOURce:SWEep:VOLTage:LINear {Vmin}, {Vmax}, {steps+1}, {delay_s}, 1, AUTO, ON, OFF" + ', "testData"')
            #self._write(f"SOURce{self._channel}:FUNCtion VOLTage")
    ...
    def takeIV(self, Vmax, step, RH, Temp):
        ...
        steps = int(Vmax//step)
        ivdata = self.voltage_sweep(1, Vmax, steps, delay_s=5.)
    ...
    def display_string(self, string):
        ...
        self._write('DISP:USER1:TEXT "{}"'.format(string))
        #self._write('DISP:USER1:TEXT:STAT ON')
        sleep(2.0)
        self._write('DISP:USER1:TEXT ""')
        #self._write('DISP:USER2:TEXT:STAT OFF')
    ...
    def takeIVnew(self, maxV, stepV, RH, Temp, errcheck_step=5):
        ...
        compl_ctr = 0
        #vltg= [100, 200, 300, 400, 500, 600, 700, 800, 900]
        for i in range(1, ln):
        #for i in range(9):    
            ...
            print("The data is ", vltg, voltage, np.abs(current),resistance)
...
class Keithley2410:
    ...
    def takeIV(self, Vmax, step, RH, Temp):
        ...
        steps = int(Vmax//step)
        ivdata = self.voltage_sweep(0.1, Vmax, steps, delay_s=5.)
